...
if [[ ! -z $USERNAME && ! -z $PASSWORD ]]; then
	/u01/app/oracle/product/11.2.0/xe/bin/sqlplus "SYSTEM/oracle" <<< "CREATE USER $USERNAME IDENTIFIED BY $PASSWORD;"
	/u01/app/oracle/product/11.2.0/xe/bin/sqlplus "SYSTEM/oracle" <<< "grant all privileges to $USERNAME identified by $PASSWORD;"
fi
